...
def _versions():
    """Every module's version, so a running device can say exactly what it is.

    Each file carries a '@v <version> | <date> | <desc>' tag and a matching
    ...
    return {
        "app": __version__,
        "main": stub,
        "my_hw": my_hw.__version__,
        "ota": ota.__version__,
        "tools": tools.__version__,
        "dashboard": DASH_VERSION,
    }

def _versions_line():
    v = _versions()
    return "app %s | my_hw %s | ota %s | tools %s | dash %s | stub %s" % (
        v["app"], v["my_hw"], v["ota"], v["tools"], v["dashboard"], v["main"])
...
class WebServer:
    ...
    def start(self):
        """Bind the port. No thread - the main loop polls us instead.

        Threads were the single biggest source of trouble in this firmware.
        _thread.start_new_thread() failed with "can't create thread" once the
        application was loaded, at every stack size down to 3 KB and with 73 KB
        of heap free - so it was never really about memory. On a bare board it
        worked fine, which is what finally showed the fault was ours, not the
        hardware's. Two boards were suspected before that became clear.

        A status page polled from the main loop is perfectly adequate: poll()
    ...
    def poll(self):
        """Serve one pending request, if any. Returns immediately otherwise."""
        if self.paused or not self.socket:
            return
        try:
        ...
        try:
            ...
            print("[WebServer] request error: %s" % e)
        finally:
            try:
                conn.close()
            except Exception:
...
def main():
    ...
    _board = config.get("device", {}).get("board", "ESP32")
    color_printer.print_color(f"Board  : {_board}", "blue")
    device_setup = DeviceSetup(_board)
    ...
    if ws_cfg.get("enabled", True):
        try:
            ...
            try:
                _tag = DASHBOARD_HTML[:120].split("@v ")[1].split(" ")[0]
                globals()["DASH_VERSION"] = _tag
            except Exception:
                pass
        except OSError:
    ...
    color_printer.print_color("Modules: " + _versions_line(), "blue")
